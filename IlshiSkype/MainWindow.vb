...
Public Class MainWindow
	...
	Private version As String = "1.3.0 (20/11/2014)"
...
#Region "Event"
		...
		If Me.dgvSounds.CurrentRow.IsNewRow = True Then
			If Me.dgvSounds.Rows.Count < ALLOWED_KEYS.Count Then
				...
				If loadingWindows.ShowDialog = Windows.Forms.DialogResult.OK Then
					...
					Me.dgvSounds.Rows.Add(chemin, Me.GetNonUsedKeys(0))
				End If
			Else
	...
	Private Sub dgvSounds_KeyColumnSelectedIndexChanged(sender As Object, e As EventArgs)
		...
		If Me.GetUsedKeys().Contains(CStr(cell.SelectedItem)) Then
	...
	Private Sub getKeyTimer_Tick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles getKeyTimer.Tick
		If Me.handlingToggleKey = False Then
			For Each key As String In Me.GetUsedKeys()
				If key IsNot Nothing Then
					If GetAsyncKeyState(AscW(key)) = -32767 Then
						'MsgBox(key)
						Me.Play(Me.GetSoundFromKey(key))
					End If
					If GetAsyncKeyState(Keys.Escape) = -32767 Then
						Me.StopPlay()
					End If
				End If
			Next
		Else
			For Each pair As KeyValuePair(Of Integer, String) In MainWindow.TOGGLE_KEYS
				If GetAsyncKeyState(pair.Key) = -32767 Then
	...
	Private Sub bgwPlayer_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles bgwPlayer.DoWork
		While Me.output.PlaybackState <> PlaybackState.Stopped
			Thread.Sleep(20)
		End While
		While Me.userOutput.PlaybackState <> PlaybackState.Stopped
			Thread.Sleep(20)
		End While
	End Sub
...
#Region "Function"
	...
	Private Function GetUsedKeys() As List(Of String)
		Return Me.dgvSounds.Rows.Cast(Of DataGridViewRow).Select(Function(n) n.Cells(Me.colPlayButton.Name).Value).Cast(Of String).ToList()
	End Function
	Private Function GetNonUsedKeys() As List(Of String)
		Dim unusedKeys As New List(Of String)(MainWindow.ALLOWED_KEYS)
		Me.GetUsedKeys().ForEach(Function(r) unusedKeys.Remove(r))
		'unusedKeys.ForEach(Sub(s) MsgBox(s)) ' TEST
		Return unusedKeys
	End Function
	Private Function GetSoundFromKey(ByVal key As String) As String
		Dim keys As List(Of String) = Me.dgvSounds.Rows.Cast(Of DataGridViewRow).Where(Function(c) c.Cells(Me.colPlayButton.Name).Value Is key).Select(Function(n) n.Cells(Me.colSoundLink.Name).Value).Cast(Of String)().ToList()
